...
class MinioConnector(models.Model):
    ...
    access_key = fields.Char(
        string="Access Key", required=True, help="MinIO Access Key ID"
    )
    secret_key = fields.Char(
        string="Secret Key", required=True, help="MinIO Secret Access Key"
    )
    secure = fields.Boolean(
        string="Use HTTPS", default=False, help="Use secure (HTTPS) connection"
    )
    region = fields.Char(
        string="Region", default="us-east-1", help="MinIO region (default: us-east-1)"
    )
    ...
    # Statistics
    total_uploads = fields.Integer(string="Total Uploads", default=0, readonly=True)
    total_downloads = fields.Integer(string="Total Downloads", default=0, readonly=True)
    ...
    def _get_minio_client(self):
        ...
        try:
            ...
            _logger.info(
                f"Creating MinIO client: endpoint={endpoint}, secure={secure}, region=None"
            )
            ...
            raise UserError(
                "Library MinIO tidak terinstall. Install dengan: pip install minio"
            )
    ...
    def action_test_connection(self):
        ...
        try:
            _logger.info(
                f"Testing MinIO connection: endpoint={self.endpoint}, secure={self.secure}"
            )
    ...
    def ensure_bucket_exists(self, bucket_name):
        ...
        try:
            ...
            _logger.info(
                f"MinIO config: endpoint={self.endpoint}, secure={self.secure}"
            )
    ...
    def upload_file(
        ...
        try:
            ...
            _logger.info(f"upload_file returning type: {type(result)} - {result}")
            return result

        except Exception as e:
            error_msg = str(e)
            _logger.error(f"Error uploading file to MinIO: {error_msg}")
            _logger.info(f"About to return dict from upload_file: success=False")
            return {"success": False, "message": f"Error upload: {error_msg}"}
    ...
    def download_file(self, bucket_name, object_name):
        ...
        try:
            ...
            file_data = response.data  # ← PERBAIKI: Gunakan .data langsung
            response.close()
            ...
            return {"success": False, "message": f"Error download: {error_msg}"}
    ...
    def delete_file(self, bucket_name, object_name):
        ...
        try:
            ...
            return {"success": False, "message": f"Error delete: {error_msg}"}
